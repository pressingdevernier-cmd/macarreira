{title: It Must Be Hell}
{artist: The Rolling Stones}
{status: en_travail}
{tags: import-kaggle, a-verifier, lang-en, british invasion, classic rock, rock}

# Import automatique 1.1.0 ; validation musicale non effectuee.
# Source: https://www.kaggle.com/datasets/eitanbentora/chords-and-lyrics-dataset
# Ligne CSV (hors en-tete): 7454
# Empreinte source SHA256: 1f0b5e3e6b23c1f3273d2457c20b05d388a76edc1f1f0b44851a818d50e122f0
# Alignement par colonnes du texte source ; aucune syllabe devinee.
# Capo: accords stockes en sons reels, formes source retrouvees par le lecteur.
# Verifications: accord_apres_fin_ligne, ligne_accords_non_reconnue, ponctuation_chordpro_neutralisee, tabulations_interpretees_4_colonnes, tonalite_absente

            TAB BY: DON CZARSKI
# SOURCE A VERIFIER: E-MAIL: [email protected] 
E-MAIL: ［email protected］

                       IT MUST BE HELL
                       ROLLING  STONES

{comment: À vérifier : INTRO RIFF: C C C C C G C C C G G G G -- 4x's}
# SOURCE A VERIFIER: INTRO RIFF:    C C C C C  G  C C C  G   G  G G -- 4x's 

          NOTE: PLAY AS BARRE CHORDS - 3rd FRET -  G  lift up
                                                  index finger

{start_of_verse: VERSE}
       [G]We got tro[C]uble, that's for s[G]ure
       We got m[C]illions unem[G]ployed
                         C                NOTE: ENTIRE SONG IS
       Some kids can't write                       ONLY THE
{comment: À vérifier : G C & G BARRE CHORDS}
# SOURCE A VERIFIER:                         G                     C & G BARRE CHORDS 
       Some kids can't read
       Some kids are h[C]ungry

                 G    INTRO RIFF - 2x's
       Some overeat

{end_of_verse}
{start_of_verse: VERSE}
       Our TV leader boldly speaks
       The words of Christ he tries to preach
       We need more power to hold the line
       The strength of darkness still abides

       Must be h[C]ell [C][C][C][C][C][G]
       L[C]iving in the world
{comment: À vérifier : G INTRO RIFF}
# SOURCE A VERIFIER:                                  G    INTRO RIFF 
       Living in the world like you

       Must be h[C]ell [C][C][C][C][C][G]
       L[C]iving in the world
{comment: À vérifier : G INTRO RIFF --2x's}
# SOURCE A VERIFIER:                                     G   INTRO RIFF --2x's 
       Suffering in the world like you

{end_of_verse}
{start_of_verse: VERSE}
       Keep in a straight line, stay in tune
       No need to worry, only fools
       End up in prison of conscience cells
       Or in asylums they help to build

{end_of_verse}
{start_of_chorus: Chorus}
       Must be hell living in the world
       Suffering in the world like you -- OPENING RIFF

       Must be hell living in the world
       Suffering in the world like you --OPENING RIFF

       Must be hell
       Suffering in the world like you -- OPENING RIFF -- 3x's

INSTRUMENTAL: SOLO OVER OPENING RIFF -- 5x's

{end_of_chorus}
{start_of_verse: VERSE}
       We're free to worship, we're free to speak
       We're free to kill, that's guaranteed
       We've got our problems, that's for sure
       Clean up the backyard, don't lock the door

{end_of_verse}
{start_of_chorus: Chorus}
       Must be hell living in the world
       Suffering in the world like you
       Must be hell living in the world
       Suffering in the world like you

OUTRO:        OPENING RIFF OVER AND OVER UTIL END
                 ( LISTEN TO CD FOR TIMING )

       I say we are heaven bound
       I say we are heaven bound
       I say we are heaven bound
       I say we are heaven bound
       I say we are heaven bound
       I say we are heaven bound
       I say we are heaven bound
       I say we are heaven bound
       I say we are heaven bound

      any questions, comments or corrections e-mail me at:
# SOURCE A VERIFIER:                    [email protected] 
                   ［email protected］

{end_of_chorus}
