{title: Going To A Go Go}
{artist: Mick Jagger}
{status: en_travail}
{tags: import-kaggle, a-verifier, lang-en, classic rock, dance rock}

# Import automatique 1.1.0 ; validation musicale non effectuee.
# Source: https://www.kaggle.com/datasets/eitanbentora/chords-and-lyrics-dataset
# Ligne CSV (hors en-tete): 103074
# Empreinte source SHA256: 556604da51d37379a0d1d750133c65e37fac1a3ec2aa26a1b7f1e9b5c190542b
# Alignement par colonnes du texte source ; aucune syllabe devinee.
# Capo: accords stockes en sons reels, formes source retrouvees par le lecteur.
# Verifications: accord_apres_fin_ligne, ligne_accords_non_reconnue, ponctuation_chordpro_neutralisee, tabulations_interpretees_4_colonnes, tonalite_absente

                     sl.
{comment: À vérifier : INTRO: / E D A E E D E???}
# SOURCE A VERIFIER: INTRO:   /  E   D A E    E D E??? 

{start_of_chorus: Chorus}
E ????????????????D    E????????????????????????????????????????????
Going to a go go, everybody
[E]Going to a go go, [D]c'mon [E]now babe
[E]Going to a go go, [D]everyb[E]ody
[E]Going to a go go, [D]c'mon [E]now

{end_of_chorus}
{start_of_verse: VERSE}
[E]Well there's a brand new pl[D]ace I f[E]ound
[E]People coming from m[D]iles ar[E]ound
[E]They come from everywh[D]ere
If you d[E]rop in th[D]ere
You'll s[E]ee everyo[D]ne in t[E]own

{end_of_verse}
{start_of_chorus: Chorus}
[E]Going to a go go, e[D]veryb[E]ody
[E]Going to a go go, c[D]'mon n[E]ow

{end_of_chorus}
{start_of_part: BREAK}
[E]Don't you wanna go [E][D][A][A][E]
[E]And that's alright [E][D][E]

{end_of_part}
{start_of_chorus: Chorus}
Going to a go go, everybody
Going to a go go

{end_of_chorus}
{start_of_verse: VERSE}
It doesn't matter if you're black
It doesn't matter if you're white
Take a dollar fifty
A six pack of beer
And we goin' dance all night

{end_of_verse}
{start_of_chorus: Chorus}
Going to a go go, everybody
Going to a go go, c'mon now

{end_of_chorus}
{start_of_part: BREAK}
Don't you wanna go
And that's alright

INSTRUMENTAL:  PLAY CHORDS TO VERSE AND Chorus

{end_of_part}
{start_of_part: BREAK}
Yeah, don't you wanna go
and that's alright

{end_of_part}
{start_of_chorus: Chorus}
Going to a go go, everybody
Going to a go go

{end_of_chorus}
{start_of_verse: VERSE}
It doesn't matter you come in drag
It doesn't matter you come in stag
I'm telling everyone to get down here
Every taxi that you flag

{end_of_verse}
{start_of_chorus: Chorus}
Is going to a go go, everybody
Going to a go go

{end_of_chorus}
{start_of_part: BREAK}
Don't you wanna go
And that's alright

{end_of_part}
{start_of_part: BREAK}
tell me [E][D][A][A][E]
And That's alright[E]  [A]   y[E]eah

         ANY QUESTIONS, COMMENTS OR CORRECTIONS E-MAIL TO:
# SOURCE A VERIFIER:                      [email protected] 
                     ［email protected］

{end_of_part}
